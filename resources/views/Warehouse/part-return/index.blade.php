        ...
        <div class="col-sm-12 col-lg-12">
          <div class="iq-card">
            <div class="iq-card-header d-flex justify-content-between">
              ...
              @if (current_user_has_permission_to('warehouse-spare_part.return.create'))
            ...
            <div class="iq-card-body">
              <form action="{{ route('warehouse.part-return.index') }}" method="GET">
                <div class="form-row">
              ...
              <div class="table-responsive">
                <table class="table table-striped table-sm" style="font-size: 12px">
                  ...
                  <tbody>
                  @forelse ($partReturns as $partReturn)
                    <tr>
                          ...
                          <div class="btn-group  btn-group-sm" role="group">
                            @if (current_user_has_permission_to('warehouse-spare_part.receiving.print'))
                              <a target="_blank" class="btn btn-success"
                                 href="{{ route('warehouse.part-return.print', $partReturn->id) }}">Print</a>
                            @endif
                            @if ($partReturn->editable() && current_user_has_permission_to('warehouse-spare_part.receiving.edit'))
                              <a class="btn btn-warning"
                                 href="{{ route('warehouse.part-return.edit', $partReturn->id) }}">Edit</a>
                              <form method="POST" id="release-{{ $partReturn->id }}"
                                    action="{{ route('warehouse.part-return.release', $partReturn->id) }}">
                                @csrf
                                @method('put')
                                <button
                                  onclick="confirmRelease({{ $partReturn->id }});"
                                  type="button" class="btn btn-primary">Complete</button>
                              </form>
                              @if (current_user_has_permission_to('warehouse-spare_part.receiving.delete'))
                                <form method="POST" id="delete-{{ $partReturn->id }}"
                                      action="{{ route('warehouse.part-return.destroy', $partReturn->id) }}">
                                  @csrf
                                  @method('delete')
                                  <button
                                    onclick="confirmDelete({{ $partReturn->id }});"
                      ...
                      <td>{{ $partReturn->number }}</td>
                      <td>{{ $partReturn->purchasing_order->number }}</td>
                      <td>{{ $partReturn->dateCreation }}</td>
                      <td>{{ $partReturn->supplier->name }}</td>
                      <td>{{ $partReturn->created_user->karyawan->nm_lengkap }}</td>
                    </tr>
                  @empty
                    <tr>
                  ...
                  <div class="d-block">
                    <div class="btn-group  btn-group-sm" role="group">
                      ...
                      <a class="btn btn-primary"
                         href="{{ route('warehouse.part-return.add', $item->id) }}">Pilih</a>
                    </div>
                  </div>
                </td>
              </tr>
