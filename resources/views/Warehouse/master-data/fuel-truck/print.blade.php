    ...
    <body>
        <table style="border-collapse: collapse; border-radius: 5px; margin: 0 auto; width: 100%" border="1" >
            <tr>
                ...
                <td><h3>FUEL TRUCK HISTORY</h3></td>
                <td>Fuel Truck Number : {{ $fuelTruck->number }}</td>
                <td>Unit : {{ $fuelTruck->equipment->name }}</td>
            </tr>
            <tr>
                <td>Capacity : {{ $fuelTruck->capacity }}</td>
                <td colspan="2">Stock : {{ $fuelTruck->stock }} </td>
            </tr>
        </table>
        <br>
        <br>
