            ...
            <td style="width: 5%; vertical-align: middle">
                <div class="btn-group" role="group" aria-label="Button group with nested dropdown">
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                            @if($list->is_draft==1)
                                <button type="button" class="dropdown-item tbl-edit-sp" data-toggle="modal" data-target="#modalFormPengajuanSP" id="{{ $list->id }}" name="btn-edit-sp[]" data-placement="top" title="Edit Pengajuan"><i class="las la-edit"></i> Edit Pengajuan</button>
                                <button type="button" class="dropdown-item tbl-cancel-sp" data-toggle="modal" data-target="#modalFormPengajuanSP" id="{{ $list->id }}" name="btn-cancel-sp[]" data-placement="top" title="Batal Pengajuan"><i class="las la-times"></i> Batal Pengajuan</button>
                            @else
...
<div id="modalFormPengajuanSP" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenteredScrollableTitle" aria-hidden="true" data-backdrop="false">
 ...
 <script type="text/javascript">
    ...
    {
        $(".list_sp").on("click", ".tbl-edit-sp", function(){
            var id_data = this.id;
            $("#v_inputan_sp").load("{{ url('hrd/suratperingatan/formEditPengajuanSP') }}/" + id_data);
        });
        $(".list_sp").on("click", ".tbl-cancel-sp", function(){
            var id_data = this.id;
            $("#v_inputan_sp").load("{{ url('hrd/suratperingatan/formDetailPengajuanSP') }}/" + id_data);
        });
