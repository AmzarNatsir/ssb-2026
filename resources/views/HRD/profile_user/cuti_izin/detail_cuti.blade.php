...
<div class="modal-body">
    <form action="{{ url('hrd/dataku/pengajuanCutiCancel/'.$res->id) }}" method="post" onsubmit="return konfirm()">
    {{ csrf_field() }}
    {{ method_field('PUT') }}
    <div class="form-group row">
        <label class="col-sm-4">Jenis Cuti</label>
        <div class="col-sm-8">
            <select class="form-control select2" name="pil_jenis_cuti" id="pil_jenis_cuti" style="width: 100%;" disabled>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <input type="text" name="inp_sisa_hak" id="inp_sisa_hak" class="form-control" value="{{ $sisa_quota }}" disabled>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-8">
            <textarea class="form-control" name="keterangan" id="keterangan" disabled>{{ $res->ket_cuti }}</textarea>
        </div>
    </div>
    <div class="form-group row">
        <label class="col-sm-4">Karyawan Pengganti</label>
        <div class="col-sm-8">
            <input type="text" name="inp_pengganti" id="inp_pengganti" class="form-control" value="{{ (empty($res->id_pengganti)) ? "" : $res->get_karyawan_pengganti->nm_lengkap }}" disabled>
        </div>
    </div>
    <hr>
...
<script type="text/javascript">
    function konfirm()
    {
        var psn = confirm("Yakin data akan membatalkan pengajuan cuti anda ?");
        if(psn==true)
        {
            return true;
        } else {
