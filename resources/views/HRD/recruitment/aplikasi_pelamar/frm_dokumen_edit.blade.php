...
<form role="form" method="post" action="{{ route('updateDokumen', $profil_dok->id) }}" enctype="multipart/form-data" onsubmit="return konfirm()">
{{ csrf_field() }}
{{ method_field('PUT') }}
<input type="hidden" name="id_pelamar" value="{{ $id_pelamar }}">
<div class="modal-body">
    <div class="form-group">
      <label for="inp_nama">Jenis Dokumen : {{ $profil_dok->get_master_dokumen->nm_dokumen }}</label>
    ...
    <div class="form-group row">
      <div class="col-sm-12">
            <img id="preview_upload_edit" style="width: 100%; height: auto;" src="{{ url(Storage::url($profil_dok->path_file.$profil_dok->file_dokumen)) }}"> 
      </div>
    </div>
</div>
