            ...
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <div class="iq-card">
                        <div class="iq-card-header d-flex justify-content-between">
                            <div class="iq-header-title">
                                <h4 class="card-title"><i class="fa fa-edit" aria-hidden="true"></i> Submit Aplikasi</h4>
                            </div>
                        </div>
                        <div class="iq-card-body" style="width:100%; height:auto">
                            <form action="{{ url('hrd/recruitment/aplikasi_pelamar/submit_app_update') }}" method="post" onsubmit="return konfirm()">
                            {{ csrf_field() }}
                                <input type="hidden" name="id_pelamar" value="{{ $profil->id }}">
                                <input type="hidden" name="status_app" value="{{ $profil->status_app }}">
                                <input type="hidden" name="status_pengajuan" value="{{ $profil->status_pengajuan }}">
                                <input type="hidden" name="key_approval" value="{{ $profil->approval_key }}">
                                @if(empty($profil->status_app))
                                <div class="form-group col-sm-12">
                                    <label for="pilPosisi">Posisi yang diamarl :</label>
                                    <select class="form-control" id="pilPosisi" name="pilPosisi">
                                        @foreach($listLowongan as $r)
                                        @if($r->id==$profil->id_lowongan)
                                        <option value="{{ $r->id }}">{{ $r->get_jabatan->nm_jabatan ." - ".$r->get_departemen->nm_dept }}</option>
                                        @endif
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <p><b>Klik Submit untuk mengaktif kembali aplikasi ini</b></p>
                                <hr>
