    ...
    <div class="col-sm-12 col-lg-12">
        <div class="iq-card">
            ...
            <div class="iq-card-body" style="width:100%; height:auto">
                <table class="table table-hover table-bordered mt-4 datatable" role="grid" aria-describedby="user-list-page-info" style="width:100%; font-size: 12px;">
                    <thead>
                        <tr>
                            <th scope="col" rowspan="2">#</th>
                            <th scope="col" rowspan="2"></th>
                            <th scope="col" rowspan="2" style="width: 20%;">Karyawan</th>
                            <th scope="col" colspan="6" style="text-align: center;">Pengajuan Cuti</th>
                    ...
                    <tbody id="result_daftar">
                        ...
                        <tr>
                            ...
                            {{ (empty($list->profil_karyawan->id_departemen)) ? "" : $list->profil_karyawan->get_departemen->nm_dept }} {{ (empty($list->profil_karyawan->id_jabatan)) ? "" : " | ".$list->profil_karyawan->get_jabatan->nm_jabatan }}
                            </td>
                            ...
                            @if ($user->can('persetujuan_cuti_approve'))
                                @if(empty($list->sts_appr_atasan))
                                    @if(date("Y-m-d") > date_format(date_create($list->tgl_awal), 'Y-m-d'))
                                        <button type="button" class="btn btn-danger" data-toggle="tooltip" data-placement="top" title="Expired"><i class="fa fa-clock-o"></i></button>
                                    @else
...
<script type="text/javascript">
    ...
    {
        var id_pengajuan = $(el).val();
        window.open("{{ url('hrd/cutiizin/formpersetujuan_hrd') }}/"+id_pengajuan);
    };
</script>
@endsection
