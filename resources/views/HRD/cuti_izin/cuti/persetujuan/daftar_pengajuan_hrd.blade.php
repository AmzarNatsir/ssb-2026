...
<div class="row">
    @if(App\Helpers\Hrdhelper::get_level_persetujuan_hrd_modul_cuti()!=auth()->user()->karyawan->id_jabatan)
                        ...
                        <tr>
                            ...
                            <td>
                                @if(empty($list->id_atasan))
                                    ...
                                    @else
                                        @if($jabatan_user == $list->profil_karyawan->get_jabatan->id_gakom)
                                        ...
                                        @else
                                        <button type="button" class="btn btn-warning" title="Menunggu Persetujuan Atasan Langsung"><i class="fa fa-hourglass-half"></i></button>
                                        @endif
                                    @endif
                                @else
                                    @if($list->sts_appr_atasan==1)
                                    <button type="button" class="btn btn-success" data-toggle="tooltip" data-placement="top" title="<?= $list->ket_appr_atasan." (by. ".$list->get_profil_atasan_langsung->nm_lengkap." - Tanggal : ".date_format(date_create($list->tgl_appr_atasan), 'd-m-Y').")" ?>"><i class="fa fa-check-circle"></i></button>
                                    @else
