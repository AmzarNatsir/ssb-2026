...
<div class="navbar-breadcrumb">
    <nav aria-label="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Tunjangan Hari Raya (THR)</li>
        <li class="breadcrumb-item"><a href="{{ url('hrd/thr_karyawan') }}">Refresh (F5)</a></li>
        </ul>
    </nav>
</div>
<div class="row">
           ...
           <div class="iq-card-body">
              <div class="d-flex justify-content-between align-items-center">
                 <div class="todo-date d-flex mr-3">
                    ...
                    <span>Data Pemberian Tunjangan Hari Raya</span>
                 </div>
                                ...
                                <table class="table" style="width: 100%">
                                    <tr style="background-color: #e9ecef">
                                        <td style="width: 40%"><h6 class="float-left mb-1"><b>Periode</b></h6><br>
                                            <h4>{{ \App\Helpers\Hrdhelper::get_nama_bulan($item->bulan) }} {{ $item->tahun }}</h4>
                                        </td>
                                        <td style="width: 40%">
                                        ...
                                        <div class="btn-group mb-1 dropup">
                                            ...
                                            <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{ url('hrd/thr_karyawan/detailThr/'.$item->id) }}"><i class="ri-table-line mr-2"></i>{{ ($item->is_draft==1 && empty($item->status_pengajuan)) ? "Pengaturan" : "Detail" }}</a>
                                                @if($item->is_draft==1 && empty($item->status_pengajuan))
                                                <a class="dropdown-item" href="{{ url('hrd/thr_karyawan/downloadtemplateThr/'.$item->tahun."/".$item->bulan) }}"><i class="fa fa-download mr-2"></i>Download Template</a>
                                                <a class="dropdown-item" href="{{ url('hrd/thr_karyawan/formImportPeriodeThr/'.$item->approval_key) }}"><i class="fa fa-upload mr-2"></i>Import</a>
                                                <a class="dropdown-item" href="{{ url('hrd/thr_karyawan/submitThr/'.$item->bulan.'/'.$item->tahun.'/'.$item->approval_key) }}"><i class="fa fa-check mr-2"></i>Submit</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
    ...
    <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered modal-ml" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalCenteredScrollableTitle">Tambah Data Periode Tunjangan Hari Raya (THR)</h5>
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        $('#form_periode').submit(function (e) {
            ...
            Swal.fire({
                title: "Yakin akan menyimpan periode THR karyawan ?",
                text: "Submit data!",
                icon: "warning",
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ url('hrd/thr_karyawan/simpanPeriodeThr') }}",
                        type: "POST",
                        headers: {
                        ...
                        success: function (response) {
                            if (response.status==true) {
                                ...
                                Swal.fire({
                                    ...
                                    window.location.href = "{{ url('hrd/thr_karyawan') }}";
                                });
                            } else {
                                Swal.fire({
                                    icon: 'error',
