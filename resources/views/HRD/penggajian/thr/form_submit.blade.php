...
<div class="navbar-breadcrumb">
    <nav aria-label="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">Form Submit</li>
        </ul>
    </nav>
</div>
...
@endif
<form id="submitPengajuan" name="submitPengajuan" method="post">
{{ csrf_field() }}
        ...
        <div class="iq-card">
            <div class="iq-card-header d-flex justify-content-between">
                <div class="iq-header-title">
                   <h4 class="card-title">Daftar Pemberian Tunjangan Hari Raya (THR) karyawan Periode : {{ $periode }} - (Preview)</h4>
                </div>
             </div>
                                        ...
                                        <table id="user-list-table-1" class="table-hover table-striped table-bordered datatable" cellpadding="4" role="grid" aria-describedby="user-list-page-info" style="width: 100%; font-size: 14px">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="text-align: center; width: 10%"">Status&nbsp;Karyawan</th>
                                                    <th style="text-align: center; width: 10%">Gaji&nbsp;Pokok</th>
                                                    <th style="text-align: center; width: 10%">Tunjangan&nbsp;Tetap</th>
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td style="text-align: center"><b>{{ $ket_status }}</b></td>
                                                    <td style="text-align: right">{{ number_format($r1['list_data']['gaji_pokok'], 0) }}</td>
                                                    <td style="text-align: right">{{ number_format($tunj_tetap, 0) }}</td>
...
<script>
    $(document).ready(function()
    {
        ...
        //     processing: true,
        // });
        $('#submitPengajuan').submit(function (e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let formData = new FormData(document.getElementById('submitPengajuan'));
                    $.ajax({
                        url: "{{ url('hrd/thr_karyawan/storeSubmitThr') }}", // Update this with your route
                        type: "POST",
                        headers: {
