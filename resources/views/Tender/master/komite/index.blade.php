      ...
      <div class="row">
        <div class="col-sm-8">
          <h4 class="card-title text-primary">
            <span class="ri-chat-check-line pr-2"></span>Master Komite
          </h4>
        </div>
        <div class="col-sm-4 text-right">
          ...
          <button id="createProjectBtn" type="button" class="btn btn-lg mb-3 btn-primary rounded-pill" data-toggle="modal" data-backdrop="static" data-target="#createProjectModal">
            <i class="las la-pen"></i>Create project
          </button>
          @endhasrole
        </div>        
      </div>
     ...
     <div  class="row">      
      ...
      <div id="action-tags" class="col-md-3 text-center d-flex align-items-center justify-content-center d-none">
        @hasrole('project_manager')
          ...
          <button 
            id="action-tag-assign-survey"
            type="button" 
            class="tag mr-2" 
            data-toggle="modal" 
            data-target="#surveyAssignmentModal">
        ...
        <div class="col-12">
          <table id="table-projects" class="table table-data nowrap w-100">
            <thead>
              <tr class="tr-shadow">
                <th>Id</th>
                <th>Nama Jenis Komite</th>
              </tr>
            </thead>
            <tbody>
              @if($komiteTypes->count() > 0)
              @foreach($komiteTypes as $key => $komite)
                <tr>
                  <td>{{ $komite->id }}</td>
                  <td>{{ $komite->name }}</td>
                </tr> 
