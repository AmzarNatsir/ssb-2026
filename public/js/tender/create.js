$(document).ready(function(){
  ...
  const uiTooltips = [{
    'selector':'#tooltip_company_name',
    'title':'dapat diisikan dengan nama perusahaan jika berupa badan usaha'
  }];
  ...
  // Tambah input upload (old)
//   $(document).on('click', '#tambah_upload', function(evt){
//     evt.preventDefault();
//     let counter = $("#wrap").children(".upload").length,
//         new_upload = $("#wrap").clone()
//           .html()
//           .replace(/\[counter\]/g, counter + 1);
//     console.log(counter);
//     $(new_upload).insertBefore("#upload_template");

//   });

    // tambah input upload (new)
    $(document).on('click', '#tambah_upload', function (evt) {
        evt.preventDefault();
        ...
        $clonedDivField.find('input[name^="filename"]').val('');
        console.log("$clonedDivField #1:", $clonedDivField.find('input[name^="filename"]').val());
        console.log("$clonedDivField: #2", $clonedDivField.find('input[name^="filename[]"]').val());
